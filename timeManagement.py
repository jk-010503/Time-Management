...
def main():
    ...
    # If there are no (valid) credentials available, user log in.
    ...
    if len(argv) < 2:
        print("Usage: python timeManagement.py <command> [duration] [description]")
        return
    ...
    if command == 'add':
        if len(argv) < 4:
            print("Usage: python timeManagement.py add <duration> <description>")
            return
        duration = argv[2]
        description = argv[3]
